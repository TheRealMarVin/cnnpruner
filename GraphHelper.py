    ...
    for torch_node in torch_graph.nodes():
        ...
        for target_torch_node in torch_graph.nodes():
            ...
            if intersect and shape is not None:
                ...
                if str(intersect) in execution_graph:
                    execution_graph[str(intersect)].extend(np.array(target_outputs))
                else:
                    execution_graph[str(intersect)] = target_outputs
                id_name_dict[str(intersect)] = curr_name
...
def clean_execution_graph(execution_graph):
    ...
    for k, v in execution_graph.items():
        for i in v:
            if str(set([i])) in execution_graph:
    ...
    for k, v in cleaned_graph.items():
        cleaned_graph[k] = str(set(cleaned_graph[k]))
