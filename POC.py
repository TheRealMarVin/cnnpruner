...
class ExecParams:
    def __init__(self, n_pretrain_epoch=10, n_epoch_retrain=3, n_epoch_total=20):
...
def common_training_code(model,
    ...
    if best_result_save_path is not None:
        if os.path.isfile(best_result_save_path):
            ...
            if not retrain_if_weight_loaded:
                should_train = False
    if should_train and exec_params.n_epoch > 0:
        local_history = train(model, optimizer, train_dataset, exec_params.n_epoch,
        ...
        exec_params.n_epoch_total = exec_params.n_epoch_total - exec_params.n_epoch
    ...
    if pruning_params.prune_ratio is not None:
        ...
        for iteration_idx in range(iterations):
            ...
            local_history = train(model, optimizer, train_dataset, exec_params.n_epoch_retrain, batch_size, use_gpu=use_gpu,
    ...
    if exec_params.n_epoch_total > 0:
        local_history = train(model, optimizer, train_dataset, exec_params.n_epoch_total,
                              batch_size, use_gpu=use_gpu, criterion=criterion,
...
def exec_alexnet(pruning_params=None, exec_params=None):
    print("***alexnet")
...
def exec_dense_net(pruning_params=None, exec_params=None):
    print("***densenet121")
...
def exec_vgg16(pruning_params=None, exec_params=None):
    print("***vgg16")
...
def exec_resnet18(pruning_params=None, exec_params=None):
    print("***resnet 18")
...
def exec_resnet34(pruning_params=None, exec_params=None):
    print("***resnet 34")
...
def exec_resnet50(pruning_params=None, exec_params=None):
    print("***resnet 50")
...
def run_startegy_prune_compare():
    ...
    exec_param_no_prune = ExecParams(n_pretrain_epoch=0, n_epoch_retrain=0, n_epoch_total=20)
    exec_param_w_prune = ExecParams(n_pretrain_epoch=10, n_epoch_retrain=3, n_epoch_total=20)
    ...
    h = exec_resnet18(pruning_params=pruning_param_no_prune, exec_params=exec_param_no_prune)
    ...
    h = exec_alexnet(pruning_params=pruning_param_no_prune, exec_params=exec_param_no_prune)
    multi_history.append_history("Alexnet 0", h)
    h = exec_alexnet(pruning_params=pruning_param_w_prune, exec_params=exec_param_w_prune)
    multi_history.append_history("Alexnet 20", h)

    h = exec_vgg16(pruning_params=pruning_param_no_prune, exec_params=exec_param_no_prune)
    multi_history.append_history("vgg16 0", h)
    h = exec_vgg16(pruning_params=pruning_param_w_prune, exec_params=exec_param_w_prune)
...
def run_alex_prune_compare():
    multi_history = MultiHistory()
    exec_param = ExecParams(n_pretrain_epoch=10, n_epoch_retrain=3, n_epoch_total=20)
    h = exec_alexnet(PruningParams(max_percent_per_iteration=0.0, prune_ratio=None), exec_params=exec_param)
    multi_history.append_history("Alexnet 0%", h)
    h = exec_alexnet(PruningParams(max_percent_per_iteration=0.05, prune_ratio=0.1), exec_params=exec_param)
    multi_history.append_history("Alexnet 10%", h)
    h = exec_alexnet(PruningParams(max_percent_per_iteration=0.15, prune_ratio=0.3), exec_params=exec_param)
    multi_history.append_history("Alexnet 30%", h)
    h = exec_alexnet(PruningParams(max_percent_per_iteration=0.25, prune_ratio=0.5), exec_params=exec_param)
    multi_history.append_history("Alexnet 50%", h)
    h = exec_alexnet(PruningParams(max_percent_per_iteration=0.25, prune_ratio=0.75), exec_params=exec_param)
